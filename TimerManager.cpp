...
void TimerManager::manageTasks()
{
	...
	while(true){
		...
		while(deltaList.size() > 0 
				&& timePassed > std::chrono::duration<double>::zero()){
			deltaList.front().timeRemaining -= timePassed;	

			if(deltaList.front().timeRemaining 
...
void TimerManager::addTask(
	...
	//	std::cout << "Packet added to empyy deltaList. Calling in "
	//		<< delay.count() << " seconds" << std::endl;
		return;
	}

	//case that there are already items in the list
	for(std::list<deltaElement>::iterator it = deltaList.begin(); 
			it != deltaList.end(); 
			++it){
		...
		if (delay < std::chrono::duration<double>::zero()){
			...
			//std::cout << "Adding new task. Will be called in " << delay.count() 
			//	<< " seconds." << std::endl;
			return;
		}
	}
